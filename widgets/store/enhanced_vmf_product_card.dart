...
class EnhancedVMFProductCard extends StatefulWidget {
  final VMFProduct product;
  final double? width;
  final bool showQuickActions;
  final bool showWishlistButton;
  final bool showCartQuantity;
  final VoidCallback? onTap;
  final VoidCallback? onAddToCart;
  final VoidCallback? onAddToWishlist;
  final VoidCallback? onRemoveFromCart;

  const EnhancedVMFProductCard({
    Key? key,
  ...
  @override
  State<EnhancedVMFProductCard> createState() => _EnhancedVMFProductCardState();
}

class _EnhancedVMFProductCardState extends State<EnhancedVMFProductCard>
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  // Barra de progreso de oferta (si aplica)
                                  if (widget.product.onSale)
                                    _buildSaleProgressBar(auraColor),
  ...
  Widget _buildSaleProgressBar(Color auraColor) {
    final now = DateTime.now();
    final endDate = widget.product.saleEndDate!;
    final totalDuration = endDate.difference(widget.product.saleStartDate ?? now);
    final remainingDuration = endDate.difference(now);
    final progress = remainingDuration.inMilliseconds / totalDuration.inMilliseconds;
    
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
          'Oferta termina en ${remainingDuration.inDays}d ${remainingDuration.inHours % 24}h',
          style: TextStyle(
            color: Colors.orange,
            fontSize: 10,
            fontWeight: FontWeight.w600,
  ...
  Widget _buildPriceAndActions(Color auraColor) {
    return Consumer<VMFCartModel>(
      builder: (context, cartModel, child) {
        final quantity = cartModel.getQuantity(widget.product.id);
        ...
        return Row(
          children: [
            ...
            // Cart quantity controls or add button
            if (widget.showCartQuantity && quantity > 0)
              _buildCartQuantityControls(quantity, auraColor)
            else
  ...
  Widget _buildCartQuantityControls(int quantity, Color auraColor) {
    return Container(
      decoration: BoxDecoration(
